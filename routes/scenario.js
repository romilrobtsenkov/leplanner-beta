...
router.post('/delete-comment/', restrict, function(req, res, next) {
  ...
  async.waterfall([
    ...
    function(next){
      ...
      var q = {};
      q.args = { _id: params.scenario._id, author: req.session.user._id };
...
router.post('/save/', restrict, function(req, res, next) {
    ...
    async.waterfall([
      function(next){
        //check author
        var q = {};
        q.args = { _id: params.scenario_data._id, author: req.session.user._id };
      ...
      function(latest_scenario, next){
        ...
        new_scenario.author = req.session.user._id;
        ...
        scenarioService.update(q, function(err, scenario){
          if (err) { return next({error: err}); }
          console.log(req.session.user.first_name+' updated scenario: '+scenario._id);
          next(null, {scenario: { _id: scenario._id } } );
        });
